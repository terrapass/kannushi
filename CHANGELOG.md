...
- Unified diff generation now limits the amount of line matching it performs on very large, highly repetitive target files, to address corner cases where it would take anomalously long to compute the diff. Diff hunk boundaries may have shifted for such files, compared to the previous **kannushi** version.

## [1.2.0][1.2.0] - 2026-07-04
...
- Support for the `--vars-duplicates` CLI argument, controlling how a top-level variable defined in more than one `--vars` file is handled: `reject` (the default, matching prior behavior) fails on any such duplicate, while `merge` combines them — concatenating duplicate lists and recursively merging duplicate dicts. A scalar value, or a list/dict type mismatch, encountered at any level is still rejected.
- `load_vars_from_yaml_files` now accepts a `duplicates_policy` argument (defaulting to the unchanged reject behavior); the `VarsDuplicatesPolicy` enum and the `DEFAULT_VARS_DUPLICATES_POLICY` constant are exposed in the package API.
...
[unreleased]: https://github.com/terrapass/kannushi/compare/v1.2.1...HEAD
