...
from os import path, cpu_count, system
...
from sys import stdout, stderr, platform as sys_platform, modules as sys_modules, path as sys_path
...
VARS_PROCESSOR_MODULE_ARG   = '--vars-processor'
VARS_PROCESSOR_FUNCTION_ARG = '--vars-processor-func'

DEFAULT_VARS_PROCESSOR_FUNCTION_NAME = 'process_vars'
...
def make_cli_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        VARS_PROCESSOR_MODULE_ARG, dest='vars_processor_module_locator', metavar='VARS_PROCESSOR_MODULE', type=str, help='Python file/module to use for variables dictionary post-processing')
    parser.add_argument(
        VARS_PROCESSOR_FUNCTION_ARG, dest='vars_processor_function_name', metavar='VARS_PROCESSOR_FUNCTION', type=str, default=DEFAULT_VARS_PROCESSOR_FUNCTION_NAME,
        help=f'single-parameter function in VARS_PROCESSOR_MODULE which vars dictionary will be passed to (defaults to {DEFAULT_VARS_PROCESSOR_FUNCTION_NAME})'
    )
...
class MainExitCode(int, Enum):
    ...
    INTERRUPTED            = SIGNAL_EXIT_CODE_OFFSET + signal.SIGINT
...
class RenderDirResult:
    ...
    def as_main_exit_code(self) -> MainExitCode:
        if self.was_interrupted:
            return MainExitCode.INTERRUPTED
        if self.errors_count > 0:
            return MainExitCode.JINJA_RENDER_ERRORS
        return MainExitCode.SUCCESS
...
def main():
    ...
    try:
        ...
        exit(MainExitCode.INTERRUPTED)
    except BaseException as e:
        print_error('\n'.join(traceback.format_exception_only(e)))
        exit(MainExitCode.VARS_LOADING_FAILED)
    ...
    if args.vars_processor_module_locator is not None:
        try:
            ...
            exit(MainExitCode.VARS_PROCESSING_FAILED)
        except ImportError as e:
            print_error(f"error: {e}")
            print(f'hint: make sure a valid Python module name or .py file path is given via {VARS_PROCESSOR_MODULE_ARG}')
            exit(MainExitCode.VARS_PROCESSING_FAILED)
            ...
            exit(MainExitCode.VARS_PROCESSING_FAILED)
    elif args.vars_processor_function_name != DEFAULT_VARS_PROCESSOR_FUNCTION_NAME:
        print_warning(f"warning: Ignoring {VARS_PROCESSOR_FUNCTION_ARG} in the absence of {VARS_PROCESSOR_MODULE_ARG}")
