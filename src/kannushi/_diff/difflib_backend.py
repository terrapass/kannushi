...
from . import TargetFileStatus, TargetDiff, determine_target_file_status
...
def difflib_diff_render_handler(context: RenderTemplateContext) -> TargetDiff:
    ...
    return TargetDiff(target_file_status, make_difflib_unified_diff(context.target_current_content, context.rendered_content, context.target_file_relative_path))

def make_difflib_unified_diff(current_content: str | None, rendered_content: str, target_file_relative_path: str) -> str:
...
def _unified_diff(from_lines: list[str], to_lines: list[str], from_file: str, to_file: str, is_junk: Callable[[str], bool] | None) -> Iterator[str]:
    ...
    started = False
    for group in matcher.get_grouped_opcodes(DIFF_CONTEXT_LINES_COUNT):
        if not started:
            started = True
            yield f'--- {from_file}\n'
            yield f'+++ {to_file}\n'
